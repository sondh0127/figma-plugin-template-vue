...
const selectedKeyState = ref('')

function onChange(selection: string) {
  selectedKeyState.value = selection
  emit('update:value', selection)
}
...
<template>
	<div class="fig-radio">
		<form class="fig-radio__wrapper" v-for="(item, i) in options" :key="i">
			<input
